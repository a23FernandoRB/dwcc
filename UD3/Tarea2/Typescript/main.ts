function diasRestantes():void {
  ...
  function cumpleanos2100():void {
   const fechaCumpleanos = new Date((document.getElementById("fechacumpleanos")as HTMLInputElement)!.value); 
...
function selectorFormato():void {
    this.event.preventDefault();
    const formatoFecha = (document.getElementById("formato")as HTMLInputElement).value;
...
function formatoHora():void{
    this.event.preventDefault();
    const fHora= (document.getElementById("formatHora")as HTMLInputElement).value;
    const fechaHoy = new Date();
