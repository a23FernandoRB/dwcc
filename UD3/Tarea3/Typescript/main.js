function getName() {
    ...
    // Sin espacios
    var sinEspacios = nombre.split(" ").join("");
    ...
    //propuesta username
    var usuario = separado[0] + separado[1].charAt(0).toUpperCase() + separado[2].charAt(0).toUpperCase();
    document.getElementById("usernamePropuesto").innerText = usuario;
}
function controlContrasena() {
    this.event.preventDefault();
    var contrasena = document.getElementById("contrasenaPropuesta").value;
    //Expresion regular de contrasena de stackoverflow
    var validacion = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])([A-Za-z\d$@$!%*?&]|[^ ]){8,15}$/;
